...
class Widget_Contents_Post_Edit extends Widget_Abstract_Contents implements Widget_Interface_Do
{
    ...
    /**
     * getResourceFieldItems
     * 
     * @access public
     * @return array
     */
    public function getResourceFieldItems()
    {
        $fields = array();
        if ($this->have()) {
            $fields = $this->db->fetchAll($this->db->select('table.resources.type, table.resources.islink, table.resources.content')
                            ->from('table.resources')
                            ->join('table.rls_resources', 'table.rls_resources.rid = table.resources.id')
                            ->where('table.rls_resources.cid = ?', $this->cid));
        }
        return $fields;
    }
    ...
    public function getBanner()
    {
        ...
        if ($this->have()) {
            $_bid = $this->db->fetchRow($this->db->select('bid')
                        ...
                        ->where('cid = ?', $this->cid));
            if(!empty($_bid)){
               $fields = $this->db->fetchRow($this->db->select('img, title')
                            ->from('table.banners')
                            ->where('bid = ?', $_bid['bid']));
            }
            /*$fields = $this->db->fetchRow($this->db->select('table.banners.img, table.banners.title')
                            ->from('table.banners')
                            ->join('table.banners', 'table.banners.bid = table.contents.bid')
    ...
    public function writePost()
    {
        ...
        $contents['title'] = $this->request->get('title', _t('咋不取个名字呢？'));
